...
for i=1:N/M
    weather=Weather(x(63)+1);
    [tao, kai, yita_high, yita_low, rou_max, sigma, am, v_free, rou_crit, alpha, T, lambda, Lm, ~, v_min, Co2, xi_r, xi_s,xi_theta,~]...
        = parameters_predict(weather);
%     rou_crit=39.5;
    %     objfun=@(theta) ModelState(x, theta, Nc, Np, para, u_pre(:,1), theta_pre(:,1));
%     confun=@(theta) ModelCons(x, theta, Nc, Np, para, u_pre(:,1));
%     theta0=cell(1,N_max);theta_t=cell(1,N_max);Fval=nan(1,N_max);Exitflag=nan(1,N_max);
%     parfor l=1:N_max
%         if l==1
%             theta0{l}=theta_pre;
%         elseif l==2
...
%     [fval, index]=min(Fval);
%     Exit=[Exit Exitflag'];
%     theta_opt=theta_t{index}(:,1);
    theta_opt=MPC_imp_RM(x,u_pre,theta_pre, Weather,rou_crit); % give the wrong weather, but rou_crit can be adjusted
...
%     uc_full=repelem(uc,1,M);
    ...
    c=[w_o0'-300;w_o1'-200;w_o2'-200;w_o3'-200];
%     u_vsl_full=u_vsl(:);
%     u_rm_full=u_rm(:);
